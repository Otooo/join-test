...
export default function BasicTableTwo({
  ...
  const [selected_item, setSelectedItem] = useState<any>();
            ...
            <TableHeader className="border-b border-gray-100 dark:border-white/[0.05]">
              <TableRow>
                {tableHeader.map((header) => {
                  if (!header.is_show) return;
                  return (
                    <TableCell key={header.title}
                      isHeader
                      className={`${header.is_action? "text-end" : " text-start"} px-5 py-3 font-medium text-gray-400 text-theme-xs dark:text-gray-500`}
                    >
                      {header.title}
                    </TableCell>
                  )
              ...
              {tableData.map((data) => (
                <TableRow key={data.id}>
                  {tableHeader.map((header) => {
                    ...
                    return (
                      !header.is_action ? (
                        <TableCell key={data.id+header.title} className="px-3 py-3 text-gray-500 text-start text-theme-sm dark:text-gray-400">
                          {data[header.value as string]}
                        </TableCell>
                      ) : (
                        <TableCell key={data.id+header.title} className="px-3 py-3 text-gray-500 text-end text-theme-sm dark:text-gray-400">
                          <div className="flex justify-end gap-5">
                            <Button onClick={() => onEdit(data)} size="sm" variant="outline" startIcon={<PencilIcon />}>
                              Editar
                            </Button>
                            <Button onClick={() => onDelete(data)} size="sm" variant="outline" startIcon={<TrashBinIcon />}>
                              Deletar
                            </Button>
                          </div>
                        </TableCell>
      ...
      >
        <div className="text-center">
          ...
          <p className="text-sm leading-6 text-gray-500 dark:text-gray-400">
            Deseja remover o seguinte item?
            <br/>
            { selected_item?.nome }
          </p>
